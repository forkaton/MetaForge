...
class DraftRepositoryImpl(
    ...
    override suspend fun pickHero(
        ...
        _draftState.update { current ->
            when {
                isBan && isAlly -> current.banAlly(slotIndex, hero)
                isBan && !isAlly -> current.banEnemy(slotIndex, hero)
                !isBan && isAlly -> current.pickAlly(slotIndex, hero)
                else -> current.pickEnemy(slotIndex, hero)
            }
        }
    }

    override suspend fun removeHero(
        slotIndex: Int,
        isAlly: Boolean,
        isBan: Boolean
    ) {
        _draftState.update { current ->
            when {
                isBan && isAlly -> current.removeAllyBan(slotIndex)
                isBan && !isAlly -> current.removeEnemyBan(slotIndex)
                !isBan && isAlly -> current.removeAlly(slotIndex)
    ...
    override suspend fun saveDraft() {
        ...
        val allyNames = state.allySlots.filterNotNull().joinToString(",") { it.name }
        val enemyNames = state.enemySlots.filterNotNull().joinToString(",") { it.name }
